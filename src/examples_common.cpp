...
vpMatrix InteractionMatrix4Points(const vpColVector &Z, const vpColVector &s){
  ...
  for(size_t i = 0; i < 4; i++){
      ...
      Lxy = InteractionMatrixPoint(Z[i], s[2*i], s[2*i+1]);
...
vpMatrix compute_Lz(const vpColVector &s,const vpColVector &Z){
  ...
  vpMatrix Lz(Z.size(),6), aux(1,6);
  for(size_t i = 0; i < Z.size(); i++) {
      aux = {0.0, 0.0, -1.0, -s[2*i+1]*Z[i], s[2*i]*Z[i], 0.0};
      Lz.insert(aux,i,0);
  }
...
vpMatrix dt_Lz(const vpColVector &s, const vpColVector &ds,const vpColVector &Z,const vpColVector &dZ){

  vpMatrix Lz(Z.size(),6), aux(1,6);
  for(size_t i = 0; i < Z.size(); i++) {
      aux = {0.0, 0.0, 0.0, -(s[2*i+1]*dZ[i] +ds[2*i+1]*Z[i]) , (s[2*i]*dZ[i]+ds[2*i]*Z[i]), 0.0};
      Lz.insert(aux,i,0);
  }
...
vpMatrix dt_InteractionMatrixPoint(const double &Z, const double &dZ, const double &x, const double &y, const double &dx, const double &dy){
  vpMatrix L(2,6);
...
vpMatrix dt_InteractionMatrix4Points(const vpColVector &Z, const vpColVector &dZ, const vpColVector &s, const vpColVector &ds){
  vpMatrix L(8,6);
  vpMatrix Lxy(2,6);
  for(size_t i = 0; i < 4; i++){

      Lxy = dt_InteractionMatrixPoint(Z[i],dZ[i], s[2*i], s[2*i+1], ds[2*i], ds[2*i+1]);
      for(size_t j = 0; j < 6; j++){
          L[2*i][j] = Lxy[0][j];
          L[2*i+1][j] = Lxy[1][j];
      }
