/*
 * Franka_cartesian_torque_control.cpp
 *
...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    vpColVector p(3,0), p_d(3,0), dx(6,0), dx_d(6,0), tau(7,0), C(7,0),gains(6,0),
                dq(7,0), tau_damped(7,0), r(3,0), err(6,0), dot_err(6,0),Eo(3,0);
    ...
    double theta = 0;
    gains = {200.0, 200.0, 200.0, 200.0, 200.0, 200.0};
    Kp.diag(gains);
    for(int i=0; i<3; i++){
...
//          std::cout << "Re: \n" << Re << "\n ----------- \n";
          ...
          if(delta_t == 0){
            ...
            tau_damped = - 0.5*(M*dq)/delta_t;
            dot_Ja = (Ja - old_Ja)/delta_t;
          }
          old_Ja = Ja;

          // Task-Frame impedance control
          tau = M*Ja.pseudoInverse()*( -Kp*err + 0*Kd*dot_err -0*dot_Ja*dq)+ C + P*tau_damped;
          ...
          if (opt_plot) {
            ...
            plotter->plot(1, current_time, P*tau_damped);
